function Gh_same = MLGF_Gh(position_s, position_f) % 用于同层的GF计算

%% ============================常量定义============================
[N,~,w,m,n,u,e,d,k] = define_H();
...
M_TM_m = @(kz) 1 ./ (1 - R_Gen_TM_down{m}(kz) .* R_Gen_TM_up{m}(kz) .* exp(2 * 1i .* k_iz(kz, m) * (d(m+1) - d(m))));
% 第一项
F1 =@(kz) exp(1i .* k_iz(kz,m) * abs(z_f - z_s));
F1_diff_zfs = @(kz) (k_iz(kz,m)).^2 .* F1(kz);
F1_diff_zf = @(kz) 1i .* k_iz(kz,m) .* sgn(z_f - z_s) .* F1(kz);
F1_diff_zs = @(kz) -F1_diff_zf(kz);
% 第二项
fv2_TE_M_zf =@(kz) exp(1i .* k_iz(kz,m) * (d(m+1) - z_f));
fr2_TE_zs =@(kz) exp(1i .* k_iz(kz, m) * (d(m+1) - z_s)) + R_Gen_TE_down{m}(kz) .* exp(1i .* k_iz(kz, m) .* (d(m+1) - 2 * d(m) + z_s));
fr2_TM_zs =@(kz) exp(1i .* k_iz(kz, m) * (d(m+1) - z_s)) + R_Gen_TM_down{m}(kz) .* exp(1i .* k_iz(kz, m) .* (d(m+1) - 2 * d(m) + z_s));

I2_TE_m =@(kz) M_TE_m(kz) .* R_Gen_TE_up{m}(kz);
I2_TM_m =@(kz) M_TM_m(kz) .* R_Gen_TM_up{m}(kz);

fv2_diff_TE_M_zf = @(kz) -1i .* k_iz(kz,m) .* fv2_TE_M_zf(kz);
fr2_diff_TM_zs = @(kz) -1i .* k_iz(kz,m) .* (exp(1i .* k_iz(kz, m) * (d(m+1) - z_s)) - R_Gen_TM_down{m}(kz) .* exp(1i .* k_iz(kz, m) .* (d(m+1) - 2 * d(m) + z_s)));

F2_TM_diff_zfs = @(kz) fv2_diff_TE_M_zf(kz) .* I2_TM_m(kz) .* fr2_diff_TM_zs(kz);
F2_TM_diff_zf = @(kz) fv2_diff_TE_M_zf(kz) .* I2_TM_m(kz) .* fr2_TM_zs(kz);
F2_TM_diff_zs = @(kz) fv2_TE_M_zf(kz) .* I2_TM_m(kz) .* fr2_diff_TM_zs(kz);
% 第三项
fv3_TE_M_zf =@(kz) exp(1i .* k_iz(kz,m) * (z_f - d(m)));
fr3_TE_zs =@(kz) exp(1i .* k_iz(kz, m) * (z_s - d(m))) + R_Gen_TE_up{m}(kz) .* exp(1i .* k_iz(kz, m) .* (2 * d(m+1) - d(m) - z_s));
fr3_TM_zs =@(kz) exp(1i .* k_iz(kz, m) * (z_s - d(m))) + R_Gen_TM_up{m}(kz) .* exp(1i .* k_iz(kz, m) .* (2 * d(m+1) - d(m) - z_s));

I3_TE_m =@(kz) M_TE_m(kz) .* R_Gen_TE_down{m}(kz);
I3_TM_m =@(kz) M_TM_m(kz) .* R_Gen_TM_down{m}(kz);

fv3_diff_TE_M_zf = @(kz) 1i .* k_iz(kz,m) .* fv3_TE_M_zf(kz);
fr3_diff_TM_zs = @(kz) 1i .* k_iz(kz,m) .* (exp(1i .* k_iz(kz, m) * (z_s - d(m))) - R_Gen_TM_up{m}(kz) .* exp(1i .* k_iz(kz, m) .* (2 * d(m+1) - d(m) - z_s)));

F3_TM_diff_zfs = @(kz) fv3_diff_TE_M_zf(kz) .* I3_TM_m(kz) .* fr3_diff_TM_zs(kz);
F3_TM_diff_zf = @(kz) fv3_diff_TE_M_zf(kz) .* I3_TM_m(kz) .* fr3_TM_zs(kz);
F3_TM_diff_zs = @(kz) fv3_TE_M_zf(kz) .* I3_TM_m(kz) .* fr3_diff_TM_zs(kz);
% 计算F和其导数
F_TE = @(kz) F1(kz) + fv2_TE_M_zf(kz) .* I2_TE_m(kz) .* fr2_TE_zs(kz) + fv3_TE_M_zf(kz) .* I3_TE_m(kz) .* fr3_TE_zs(kz);
F_TM = @(kz) F1(kz) + fv2_TE_M_zf(kz) .* I2_TM_m(kz) .* fr2_TM_zs(kz) + fv3_TE_M_zf(kz) .* I3_TM_m(kz) .* fr3_TM_zs(kz);


F_TM_diff_zfs = @(kz) F1_diff_zfs(kz) + F2_TM_diff_zfs(kz) +F3_TM_diff_zfs(kz);
F_TM_diff_zf = @(kz) F1_diff_zf(kz) + F2_TM_diff_zf(kz) + F3_TM_diff_zf(kz);
F_TM_diff_zs = @(kz) F1_diff_zs(kz) + F2_TM_diff_zs(kz) + F3_TM_diff_zs(kz);
%% ============================计算G============================
G_TE = zeros(3,3,length(x_f));
G_TM = zeros(3,3,length(x_f));
Gh_same = zeros(3,3,length(x_f));
...
for i = 1:length(x_f)
    ...
    f_TE_s0 = @(kz) F_TE(kz) ./ (k_iz(kz, m)+1) .* besselj(0, kz * rho) .* kz;  % TE波0阶Sommerfield积分被积函数
    ...
    Gh_same(:,:,i) = G_TE(:,:,i) + G_TM(:,:,i)/k_nm^2;
end
